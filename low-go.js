...
function removeVideosFromElement(node) {
  let elements = Array.prototype.slice.call(
    node.getElementsByTagName("ytd-rich-item-renderer"),
    0,
  );
  let elements_ = Array.prototype.slice.call(
    node.getElementsByTagName("ytd-compact-video-renderer"),
    0,
  );
  let allElements = elements.concat(elements_);
  for (let i = 0; i < allElements.length; i++) {
    let views = allElements[i].getElementsByClassName(
...
function addObservers() {
  let recommendedVideos = document.querySelector(
    "div#primary ytd-rich-grid-renderer div#contents",
  );
